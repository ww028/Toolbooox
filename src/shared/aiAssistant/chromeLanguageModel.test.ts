...
describe("chrome language model", () => {
  ...
  it("builds an optimized prompt with task rules, examples, and recent context", () => {
    ...
    expect(prompt).toContain("回答是/否或事实核验问题时按三档处理");
    expect(prompt).toContain("通用常识或职业/身份常识合理推出");
    expect(prompt).toContain("不是已记录事实");
    expect(prompt).toContain("不要把推断说成确定事实");
    expect(prompt).toContain("前端开发工程师」通常需要 JavaScript");
    ...
    expect(prompt).toContain("约为 2016 年，而不是 2023 年");
    expect(prompt).toContain("如果没有明确时间关系，只能说明缺少信息");
    expect(prompt).toContain("当前日期：");
    expect(prompt).toContain("不要在最终回答里说“根据片段”");
    expect(prompt).toContain("既没有直接证据也不能可靠推断时，再说明不知道");
